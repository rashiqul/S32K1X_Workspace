...
{
    S32_NVIC->ISER[FEATURE_NVIC_REGISTER_INDEX(IRQn)] =
        (uint32)(1UL << ((uint32)(IRQn) & (uint32)0x1FU));
}
...
{
    S32_NVIC->ICER[FEATURE_NVIC_REGISTER_INDEX(IRQn)] =
        (uint32)(1UL << ((uint32)(IRQn) & (uint32)0x1FU));
}
...
void NVIC_SetPriority(uint8 IRQn, uint8 priority)
{
    ...
    S32_NVIC->IP[iprVectorId] |= ((uint32)(((((uint32)priority) << shift)) & 0xFFUL))
                                 << priByteShift;
#endif
}
