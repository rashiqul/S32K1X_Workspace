...
typedef enum {
    ...
    CLOCK_MONITOR_NOTIFICATIONS =
        0U, /* Notification is sent on behalf of interrupt service routine from power */
...
typedef enum {
    CLOCK_IP_LOWEST_LATENCY =
        0U, /**< @brief Clock monitor is configured to react as fast as possible. A clock failure is
               detected by clock monitor unit in less than one microsecond. */
    CLOCK_IP_LOW_LATENCY_1US = 1U, /**< @brief Clock monitor reaction is taking 1us. A clock failure
                                      is detected by clock monitor unit in one microsecond. */
    CLOCK_IP_MEDIUM_LATENCY_10US =
        10U, /**< @brief Clock monitor reaction is taking 10us. A clock failure is detected by clock
                monitor unit in 10 microseconds. */
    CLOCK_IP_HIGH_LATENCY_100US =
        100U, /**< @brief Clock monitor reaction is taking 100us. A clock failure is detected by
                 clock monitor unit in 100 microseconds. */
    CLOCK_IP_VERY_HIGH_LATENCY_1MS =
        1000U, /**< @brief Clock monitor reaction is taking 1ms. A clock failure is detected by
                  clock monitor unit in 1 milisecond. */
 ...
 */
typedef void (*Clock_Ip_NotificationsCallbackType)(Clock_Ip_NotificationType Error,
...
typedef struct {
    Clock_Ip_NameType
        Name; /**< Clock name associated to divider for which trigger is configured. */
    Clock_Ip_TriggerDividerType TriggerType; /**< Trigger value - if value is zero then divider is
                                                updated immediately, divider is not triggered. */
    Clock_Ip_NameType Source; /**< Clock name of the common input source of all dividers from the
                                 same group that support a common update */
...
typedef struct {
    Clock_Ip_NameType
        Name; /**<  Clock source from which ramp-down and to which ramp-up are processed. */
    uint32 MaxAllowableIDDchange;   /**<  Maximum variation of current per time (mA/microsec) -  max
                                       allowable IDD change is determined by the user's power supply
                                       design. */
    uint32 StepDuration;            /**<  Step duration of each PCFS step */
    Clock_Ip_NameType SelectorName; /**<  Name of the selector that supports PCFS and name is one
                                       the inputs that can be selected */
    uint32 ClockSourceFrequency;    /**<  Frequency of the clock source from which ramp-down and to
                                       which ramp-up are processed. */
...
typedef struct {
    ...
    uint32 MonitoredClockFrequency; /**< Frequency of the clock source from which ramp-down and to
                                       which ramp-up are processed. */
    Clock_Ip_RegisterIndexType
        Indexes; /**< Register index if register value optimization is enabled. */
...
typedef struct {
    ...
    Clock_Ip_DividerTriggerConfigType
        DividerTriggers[CLOCK_IP_DIVIDER_TRIGGERS_NO]; /**< Divider triggers */
    Clock_Ip_FracDivConfigType
        FracDivs[CLOCK_IP_FRACTIONAL_DIVIDERS_NO];           /**< Fractional dividers */
    ...
    Clock_IP_SpecificPeriphConfigType
        SpecificPeriphalConfiguration; /**< Clock specific peripheral configuration */

    Clock_Ip_ConfiguredFrequencyType
        ConfiguredFrequencies[CLOCK_IP_CONFIGURED_FREQUENCIES_NO]; /**< Configured frequency values
                                                                    */
