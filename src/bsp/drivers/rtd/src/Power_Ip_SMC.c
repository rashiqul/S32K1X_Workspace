...
/* Check if Power_Ip_SMC.c file and Power_Ip_CortexM4.h file are of the same Autosar version */
     ...
     (POWER_IP_SMC_AR_RELEASE_REVISION_VERSION_C !=                                                \
 ...
 * @details      The function get status of Very Low Power Stop Aborted bit in SMC_PMCTRL register
 * when processor enter VLPS. Function return POWER_IP_STOP_SUCCESSFUL if enter VLPS was
 * successfully, POWER_IP_STOP_ABORTED if enter VLPS was Aborted.
 *
 * @param[in]    none
 *
...
uint32 Power_Ip_SMC_ModeConfig(const Power_Ip_ModeConfigType* ModeConfigPtr)
{
    ...
    case (POWER_IP_VLPS_MODE): {
        ...
        if (POWER_IP_STOP_ABORTED == Power_Ip_SMC_Get_VLPSA_Status()) {
            Power_Ip_ReportPowerErrors(POWER_IP_REPORT_VLPSA_NOTIFICATION,
                                       POWER_IP_ERR_CODE_RESERVED);
        }
#endif
        break;
    }
