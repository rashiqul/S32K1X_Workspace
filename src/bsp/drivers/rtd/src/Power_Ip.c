...
static const Power_Ip_ResetType ResetReasonArray[(uint8)MCU_RESET_UNDEFINED + 1U] = {
...
Power_Ip_ReportErrorsCallbackType Power_Ip_pfReportErrorsCallback =
...
#if (POWER_IP_ENTER_LOW_POWER_MODE == STD_ON)
        if ((POWER_IP_VLPS_MODE == PowerMode) || (POWER_IP_STOP1_MODE == PowerMode) ||
            (POWER_IP_STOP2_MODE == PowerMode)) {
...
Power_Ip_ResetType Power_Ip_GetResetReason(void)
{
    ...
    /* Use function Power_Ip_ConvertIntergeToResetType to avoid MISRA violation 10.5 : cast from
     * uint32 to enum */
