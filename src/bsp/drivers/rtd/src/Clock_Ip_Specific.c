...
/* Check if Clock_Ip_Specific.c file and Clock_Ip_Private.h file are of the same Autosar version */
#if ((CLOCK_IP_SPECIFIC_AR_RELEASE_MAJOR_VERSION_C !=                                              \
     ...
     (CLOCK_IP_SPECIFIC_AR_RELEASE_REVISION_VERSION_C !=                                           \
...
static boolean Clock_Ip_bFircWasEnabledBeforeMcuInit =
    TRUE; /* Firc clock was enabled before MCU initialization */
...
static void DisableSafeClock(Clock_Ip_ClockConfigType const* Config)
{
        ...
        for (Index = 0U; Index < Clock_Ip_apConfig->IrcoscsCount; Index++) {
            if (Clock_Ip_apConfig->Ircoscs[Index].Name == FIRC_CLK) {
                ...
                if (Clock_Ip_apConfig->Ircoscs[Index].Enable == FALSE) {
                    ...
                    Clock_Ip_axIntOscCallbacks[Clock_Ip_au8IrcoscCallbackIndex[CLOCK_IP_FIRCOSC]]
                        .Disable(FIRC_CLK);
                }
                break;
            }
    ...
    if ((FircConfigFound == FALSE) && (Clock_Ip_bFircWasEnabledBeforeMcuInit == FALSE)) {
        /* Disable FIRC according to configuration */
        Clock_Ip_axIntOscCallbacks[Clock_Ip_au8IrcoscCallbackIndex[CLOCK_IP_FIRCOSC]].Disable(
            FIRC_CLK);
    }
}
...
void SetSimLpoclksRegister_TrustedCall(Clock_Ip_ClockConfigType const* Config)
{
    ...
    for (Index = 0U; Index < Config->SelectorsCount; Index++) {
        ...
        if (RTC_CLK == Config->Selectors[Index].Name) {
            SimLpoValue |=
                ((uint32)(Clock_Ip_au16SelectorEntryHardwareValue[Config->Selectors[Index].Value])
        ...
        if (LPO_CLK == Config->Selectors[Index].Name) {
            SimLpoValue |=
                ((uint32)(Clock_Ip_au8SelectorEntrySIMHardwareValue[Config->Selectors[Index].Value])
         ...
         Index++) /* Set clock gates that are under clock control. */
    {
        ...
        if (LPO_32K_CLK == Config->Gates[Index].Name) {
            SimLpoValue =
                (SimLpoValue & (~SIM_LPOCLKS_LPO32KCLKEN_MASK)) |
                (((uint32)(Config->Gates[Index].Enable) << SIM_LPOCLKS_LPO32KCLKEN_SHIFT) &
                 SIM_LPOCLKS_LPO32KCLKEN_MASK);
        }

        /* Gate for LPO_1K_CLK */
        if (LPO_1K_CLK == Config->Gates[Index].Name) {
            SimLpoValue = (SimLpoValue & (~SIM_LPOCLKS_LPO1KCLKEN_MASK)) |
...
static void Clock_Ip_SpecificPlatformInitClock(Clock_Ip_ClockConfigType const* Config)
{
    ...
    CLOCK_IP_DEV_ASSERT(CLOCK_IP_RUN_POWER_MODE_STATUS ==
...
static const Clock_Ip_IrcoscConfigType* getFircConfig(void)
{
    ...
    if (ReturnValue == NULL_PTR) {
        ...
        FircConfiguration.Enable =
            (uint16)(IP_SCG->FIRCCSR & SCG_FIRCCSR_FIRCEN_MASK) >> SCG_FIRCCSR_FIRCEN_SHIFT;
        FircConfiguration.Range =
            (uint8)(IP_SCG->FIRCCFG & SCG_FIRCCFG_RANGE_MASK) >> SCG_FIRCCFG_RANGE_SHIFT;
        FircConfiguration.Regulator =
            (uint8)(IP_SCG->FIRCCSR & SCG_FIRCCSR_FIRCREGOFF_MASK) >> SCG_FIRCCSR_FIRCREGOFF_SHIFT;
    }
...
static const Clock_Ip_XoscConfigType* getSoscConfig(void)
{
    ...
    if (ReturnValue == NULL_PTR) {
        ...
        SoscConfiguration.Enable =
            (uint16)(IP_SCG->SOSCCSR & SCG_SOSCCSR_SOSCEN_MASK) >> SCG_SOSCCSR_SOSCEN_SHIFT;
...
static const Clock_Ip_PllConfigType* getSpllConfig(void)
{
    ...
    if (ReturnValue == NULL_PTR) {
        ...
        SpllConfiguration.Enable =
            (uint16)(IP_SCG->SPLLCSR & SCG_SPLLCSR_SPLLEN_MASK) >> SCG_SPLLCSR_SPLLEN_SHIFT;
        SpllConfiguration.Predivider =
            (uint8)((IP_SCG->SPLLCFG & SCG_SPLLCFG_PREDIV_MASK) >> SCG_SPLLCFG_PREDIV_SHIFT);
        SpllConfiguration.MulFactorDiv =
            (uint8)((IP_SCG->SPLLCFG & SCG_SPLLCFG_MULT_MASK) >> SCG_SPLLCFG_MULT_SHIFT);
    }
    ...
    if (ReturnValue == NULL_PTR) {
        ...
        case FIRC_MON1_CLK:
            ...
            if (((IP_PCC->PCCn[62] & PCC_PCCn_CGC_MASK) >> PCC_PCCn_CGC_SHIFT) == 1U) {
                CmuConfiguration.Enable =
                    (Clock_Ip_apxCmu[0U]->GCR & CMU_FC_GCR_FCE_MASK) >> CMU_FC_GCR_FCE_SHIFT;
            }
            break;
        case FIRC_MON2_CLK:
            ...
            if (((IP_PCC->PCCn[63] & PCC_PCCn_CGC_MASK) >> PCC_PCCn_CGC_SHIFT) == 1U) {
                CmuConfiguration.Enable =
                    (Clock_Ip_apxCmu[1U]->GCR & CMU_FC_GCR_FCE_MASK) >> CMU_FC_GCR_FCE_SHIFT;
            }
            break;
        default:
            /* Invalid clock name */
...
static const Clock_Ip_SelectorConfigType* getSelectorConfig(Clock_Ip_NameType Name)
{
    ...
    if (ReturnValue == NULL_PTR) {
        ...
        switch (Name) {
        case SCS_RUN_CLK:
            SelectorConfigurations[SelectorConfigIndex].Value =
                ClockSource[((IP_SCG->RCCR & SCG_RCCR_SCS_MASK) >> SCG_RCCR_SCS_SHIFT) & 0x7U];
            break;
        case SCS_VLPR_CLK:
            SelectorConfigurations[SelectorConfigIndex].Value =
                ClockSource[(IP_SCG->VCCR & SCG_VCCR_SCS_MASK) >> SCG_VCCR_SCS_SHIFT];
            break;
#if defined(CLOCK_IP_HAS_SCS_HSRUN_CLK)
        case SCS_HSRUN_CLK:
            SelectorConfigurations[SelectorConfigIndex].Value =
                ClockSource[(IP_SCG->HCCR & SCG_HCCR_SCS_MASK) >> SCG_HCCR_SCS_SHIFT];
            break;
#endif
        default:
            /* Invalid clock name */
...
static const Clock_Ip_DividerConfigType* getCoreDividerConfig(Clock_Ip_NameType Name)
{
    ...
    if (ReturnValue == NULL_PTR) {
        ...
        switch (Name) {
        case CORE_RUN_CLK:
            CoreDividerConfigurations[DividerConfigIndex].Value =
                ((IP_SCG->RCCR & SCG_RCCR_DIVCORE_MASK) >> SCG_RCCR_DIVCORE_SHIFT) + 1U;
            break;
        case CORE_VLPR_CLK:
            CoreDividerConfigurations[DividerConfigIndex].Value =
                ((IP_SCG->VCCR & SCG_VCCR_DIVCORE_MASK) >> SCG_VCCR_DIVCORE_SHIFT) + 1U;
            break;
#if defined(CLOCK_IP_HAS_CORE_HSRUN_CLK)
        case CORE_HSRUN_CLK:
            CoreDividerConfigurations[DividerConfigIndex].Value =
...
static const Clock_Ip_DividerConfigType* getBusDividerConfig(Clock_Ip_NameType Name)
{
    ...
    if (ReturnValue == NULL_PTR) {
        ...
        switch (Name) {
        case BUS_RUN_CLK:
            BusDividerConfigurations[DividerConfigIndex].Value =
                ((IP_SCG->RCCR & SCG_RCCR_DIVBUS_MASK) >> SCG_RCCR_DIVBUS_SHIFT) + 1U;
            break;
        case BUS_VLPR_CLK:
            BusDividerConfigurations[DividerConfigIndex].Value =
                ((IP_SCG->VCCR & SCG_VCCR_DIVBUS_MASK) >> SCG_VCCR_DIVBUS_SHIFT) + 1U;
            break;
#if defined(CLOCK_IP_HAS_BUS_HSRUN_CLK)
        case BUS_HSRUN_CLK:
            BusDividerConfigurations[DividerConfigIndex].Value =
...
static const Clock_Ip_DividerConfigType* getSlowDividerConfig(Clock_Ip_NameType Name)
{
    ...
    if (ReturnValue == NULL_PTR) {
        ...
        switch (Name) {
        case SLOW_RUN_CLK:
            SlowDividerConfigurations[DividerConfigIndex].Value =
                ((IP_SCG->RCCR & SCG_RCCR_DIVSLOW_MASK) >> SCG_RCCR_DIVSLOW_SHIFT) + 1U;
            break;
        case SLOW_VLPR_CLK:
            SlowDividerConfigurations[DividerConfigIndex].Value =
                ((IP_SCG->VCCR & SCG_VCCR_DIVSLOW_MASK) >> SCG_VCCR_DIVSLOW_SHIFT) + 1U;
            break;
#if defined(CLOCK_IP_HAS_SLOW_HSRUN_CLK)
        case SLOW_HSRUN_CLK:
            SlowDividerConfigurations[DividerConfigIndex].Value =
...
#if defined(CLOCK_IP_HAS_SPLL_CLK)
        Clock_Ip_pxSpllClock =
            &Clock_Ip_axPllCallbacks[Clock_Ip_au8PllCallbackIndex[CLOCK_IP_SYS_PLL]];
#endif

        Clock_Ip_pxSoscClock =
            &Clock_Ip_axExtOscCallbacks[Clock_Ip_au8XoscCallbackIndex[CLOCK_IP_SYS_OSC]];

        Clock_Ip_pxFircClock =
            &Clock_Ip_axIntOscCallbacks[Clock_Ip_au8IrcoscCallbackIndex[CLOCK_IP_FIRCOSC]];

#if defined(CLOCK_IP_HAS_FIRC_MON1_CLK) || defined(CLOCK_IP_HAS_FIRC_MON2_CLK)
        Clock_Ip_pxCmuFircMonitor =
            &Clock_Ip_axCmuCallbacks[Clock_Ip_au8CmuCallbackIndex[CLOCK_IP_CMU]];
#endif

        Clock_Ip_pxScsRunClockSelector =
            &Clock_Ip_axSelectorCallbacks[Clock_Ip_au8SelectorCallbackIndex[CLOCK_IP_SCS_RUN]];
#if defined(CLOCK_IP_HAS_SCS_HSRUN_CLK)
        Clock_Ip_pxScsHsrunClockSelector =
            &Clock_Ip_axSelectorCallbacks[Clock_Ip_au8SelectorCallbackIndex[CLOCK_IP_SCS_HSRUN]];
#endif

        Clock_Ip_pxCoreRunClockDivider =
            &Clock_Ip_axDividerCallbacks[Clock_Ip_au8DividerCallbackIndex[CLOCK_IP_DIVCORE_RUN]];
#if defined(CLOCK_IP_HAS_CORE_HSRUN_CLK)
        Clock_Ip_pxCoreHsrunClockDivider =
            &Clock_Ip_axDividerCallbacks[Clock_Ip_au8DividerCallbackIndex[CLOCK_IP_DIVCORE_HSRUN]];
#endif

        Clock_Ip_pxBusRunClockDivider =
            &Clock_Ip_axDividerCallbacks[Clock_Ip_au8DividerCallbackIndex[CLOCK_IP_DIVBUS_RUN]];
#if defined(CLOCK_IP_HAS_BUS_HSRUN_CLK)
        Clock_Ip_pxBusHsrunClockDivider =
            &Clock_Ip_axDividerCallbacks[Clock_Ip_au8DividerCallbackIndex[CLOCK_IP_DIVBUS_HSRUN]];
#endif

        Clock_Ip_pxSlowRunClockDivider =
            &Clock_Ip_axDividerCallbacks[Clock_Ip_au8DividerCallbackIndex[CLOCK_IP_DIVSLOW_RUN]];
#if defined(CLOCK_IP_HAS_SLOW_HSRUN_CLK)
        Clock_Ip_pxSlowHsrunClockDivider =
            &Clock_Ip_axDividerCallbacks[Clock_Ip_au8DividerCallbackIndex[CLOCK_IP_DIVSLOW_HSRUN]];
#endif
    }

    if (((TRUE == Clock_Ip_bAcceptedCopyClockConfiguration) && (Config == NULL_PTR)) ||
    ...
    if (Notification != CLOCK_MONITOR_NOTIFICATIONS) {
        ...
        case RUN_MODE: {
            if (POWER_MODE_CHANGED == Notification) {
                ...
                (void)(Clock_Ip_pxSpllClock->Complete(
                    Clock_Ip_pxSpllConfig->Name)); /* Wait to lock */
#endif
