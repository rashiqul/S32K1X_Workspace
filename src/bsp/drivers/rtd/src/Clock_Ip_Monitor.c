...
/* Check if Clock_Ip_Monitor.c file and Clock_Ip_Private.h file are of the same Autosar version */
     ...
     (CLOCK_IP_MONITOR_AR_RELEASE_REVISION_VERSION_C !=                                            \
...
static void Clock_Ip_DisableCmuFcFceRefCntLfrefHfref(Clock_Ip_NameType Name)
{
    const Clock_Ip_CmuInfoType* CmuInformation =
        &Clock_Ip_axCmuInfo[Clock_Ip_au8ClockFeatures[Name][CLOCK_IP_CMU_INDEX]];
    ...
    if (CLOCK_IP_CMU_FREQUENCY_CHECK_ENABLED == (CmuFc->GCR & CMU_FC_GCR_FCE_MASK)) {
        ...
        } while ((CLOCK_IP_CMU_FREQUENCY_CHECK_STOPPED == FrequencyCheckStatus) &&
                 (!TimeoutOccurred));
    ...
    /* Disable interupts */
    CmuFc->IER &= ~(CMU_FC_IER_FLLIE_MASK | CMU_FC_IER_FHHIE_MASK | CMU_FC_IER_FLLAIE_MASK |
                    CMU_FC_IER_FHHAIE_MASK);
...
static void Clock_Ip_SetCmuFcFceRefCntLfrefHfref(Clock_Ip_CmuConfigType const* Config, uint32 Index)
{
    ...
    const Clock_Ip_CmuInfoType* CmuInformation =
        &Clock_Ip_axCmuInfo[Clock_Ip_au8ClockFeatures[Config->Name][CLOCK_IP_CMU_INDEX]];

    if ((NULL_PTR != Config) && (Config->Enable != 0U)) {
        /* Do not calculate cmu values if these values are already calculated and written in hw
         * registers */
        if (HashCmu[Index] !=
            ((((uint32)Config->MonitoredClockFrequency) ^ ((uint32)Config->Name)))) {
            HashCmu[Index] = ((((uint32)Config->MonitoredClockFrequency) ^ ((uint32)Config->Name)));

            ReferenceClk =
                (Clock_Ip_pxConfig
                     ->ConfiguredFrequencies)[Clock_Ip_FreqIds[CmuInformation->Reference]]
                    .ConfiguredFrequencyValue;
            BusClk =
                (Clock_Ip_pxConfig->ConfiguredFrequencies)[Clock_Ip_FreqIds[CmuInformation->Bus]]
                    .ConfiguredFrequencyValue;
            MonitoredClk = Config->MonitoredClockFrequency;

#if (defined(CLOCK_IP_DEV_ERROR_DETECT) && (CLOCK_IP_DEV_ERROR_DETECT == STD_ON))
            CLOCK_IP_DEV_ASSERT((ReferenceClk != 0U) && (ReferenceClk < 65536000) &&
                                (BusClk != 0U) && (MonitoredClk != 0U));
#endif
            ...
            if ((ReferenceClk != 0U) && (BusClk != 0U) && (MonitoredClk != 0U)) {
                ...
                case CLOCK_IP_VERY_HIGH_LATENCY_1MS: {
                    /* Example: FXOSC frequency is 40MHz, then counter is 40 for 1us latency, 400
                     * for 10us latency, 4000 for 100us latency and 40000 for 1ms latency. */
                    /* If reference clock frequency is 40 MHz, and monitor latency is 1us (counter =
                     * 40000000Hz/1000000), 10us (counter = 40000000Hz/100000), 100us (counter =
                     * 40000000Hz/10000), 1ms (counter = 40000000Hz/1000) */
                    RefCount = ReferenceClk / (1000000U / Config->Latency);
                    RefCount = ((Cmp1 > Cmp2) ? Cmp1 : Cmp2) > RefCount
                                   ? ((Cmp1 > Cmp2) ? Cmp1 : Cmp2)
                                   : RefCount;
                } break;
                ...
                /* Formula for Hfref
                   ceil((fMonitoredClk mul MonitoredClockPlusVariation mul RefCount) div
                        (fReferenceClk mul ReferenceClockMinusVariation))
                         + CLOCK_IP_CMU_FC_VAR
                 */
                HfRef = (((((uint64)MonitoredClk) * ((uint64)(10000U + Config->Variation)) *
                           ((uint64)RefCount)) -
                          1UL) /
                         (((uint64)ReferenceClk) *
                          ((uint64)(10000U - CmuInformation->FreqRefVariation)))) +
                        1UL + CLOCK_IP_CMU_FC_VAR;
                ...
                /* Formula for LfRef
                   floor((fMonitoredClk mul MonitoredClockMinusVariation mul RefCount) div
                        (fReferenceClk mul ReferenceClockPlusVariation))
                         - CLOCK_IP_CMU_FC_VAR
                 */
                LfRef = ((((uint64)MonitoredClk) * ((uint64)(10000U - Config->Variation)) *
                          ((uint64)RefCount)) /
                         (((uint64)ReferenceClk) *
                          ((uint64)(10000U + CmuInformation->FreqRefVariation)))) -
                        CLOCK_IP_CMU_FC_VAR;
                ...
                ((Clock_Ip_ClockMonitorType*)CmuInformation->CmuInstance)->HTCR =
                    (((uint32)HfRef) > (uint32)0xFFFFFC) ? (uint32)0xFFFFFC : ((uint32)HfRef);

                /* Set low limit. Do not program LFREF to a value less than 0x00000003 */
                ((Clock_Ip_ClockMonitorType*)CmuInformation->CmuInstance)->LTCR =
                    (((uint32)LfRef) < (uint32)3U) ? (uint32)3U : ((uint32)LfRef);
            }
        }
 ...
 * @details      Called by RGM ISR routine when a user notification for CMU FCCU events is
 * configured
 *
 * @return       void
 *
