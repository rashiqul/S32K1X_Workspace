...
#define GPIO_DIO_REVERSE_BITS_NUMBER                                                               \
    ((Dio_PortLevelType)((Dio_PortLevelType)((Dio_PortLevelType)(sizeof(Dio_PortLevelType))        \
                                             << 0x3U) -                                            \
                         1U))
...
static Dio_PortLevelType Dio_Ipw_ReverseBits(Dio_PortLevelType Level)
{
    ...
    Dio_PortLevelType NumShifts =
        GPIO_DIO_REVERSE_BITS_NUMBER; /* number of shifts needed due to trailing 0s */

    for (TempLevel = (Dio_PortLevelType)(TempLevel >> GPIO_DIO_REVERSE_BITS_SHIFT);
         (Dio_PortLevelType)STD_LOW != TempLevel;
         TempLevel = (Dio_PortLevelType)(TempLevel >> GPIO_DIO_REVERSE_BITS_SHIFT)) {
        RevLevel = (Dio_PortLevelType)(RevLevel << GPIO_DIO_REVERSE_BITS_SHIFT);
        RevLevel =
            (Dio_PortLevelType)(RevLevel |
                                ((Dio_PortLevelType)(TempLevel & GPIO_DIO_REVERSE_BITS_MASK)));
        NumShifts--;
    }

    RevLevel =
        (Dio_PortLevelType)((uint32)RevLevel
                            << NumShifts); /* Remaining shifts when Level's highest bits are zero */
...
Dio_LevelType Dio_Ipw_FlipChannel(Dio_ChannelType ChannelId)
{
    ...
    ChannelLevel =
        (Dio_LevelType)((u32PortOutPutLevel & ((uint32)1U << u32PinIndex)) >> u32PinIndex);
...
#if (STD_ON == DIO_REVERSEPORTBITS)
    PortLevel = (Dio_PortLevelType)((pinsValue & (pChannelGroupIdPtr->mask))
                                    << (pChannelGroupIdPtr->u8offset));
    PortLevel = (Dio_PortLevelType)(Dio_Ipw_ReverseBits(PortLevel));
#else
    PortLevel = (Dio_PortLevelType)((pinsValue & (pChannelGroupIdPtr->mask)) >>
                                    (pChannelGroupIdPtr->u8offset));
...
#if (STD_ON == DIO_REVERSEPORTBITS)
    ValueSet =
        (((Dio_PortLevelType)(Dio_Ipw_ReverseBits(Level)) >> (pChannelGroupIdPtr->u8offset)) &
         (((Dio_PortLevelType)(pChannelGroupIdPtr->mask))));
#else
    ValueSet = (((Dio_PortLevelType)(Level) << (pChannelGroupIdPtr->u8offset)) &
                (((Dio_PortLevelType)(pChannelGroupIdPtr->mask))));
#endif
