...
/* Check if Mcu_Dem_Wrapper.c file and Mcu_Dem_Wrapper.h file are of the same Autosar version */
     ...
     (MCU_DEM_WRAPPER_AR_RELEASE_REVISION_VERSION_C !=                                             \
      MCU_DEM_WRAPPER_AR_RELEASE_REVISION_VERSION))
...
void Mcu_ReportDemTimeoutError(void)
{
    if ((uint32)STD_ON == Mcu_pDemCfgPtr->ErrorTimeoutFailureCfg.state) {
        /* According to requirement [SWS_Dem_00183] from the ASR 4.4
         * "AUTOSAR_SWS_DiagnosticEventManager" document, "BSW modules calling Dem_SetEventStatus
         * can safely ignore the return value." */
        (void)Dem_SetEventStatus((Dem_EventIdType)Mcu_pDemCfgPtr->ErrorTimeoutFailureCfg.id,
...
/**
 * @brief    Reports timeout error to DEM.
 * @details  Checks if the switch mode failure and reports the timeout error to DEM if that is the
 * case.
 *
 * @param[in]  void.
 *
...
void Mcu_ReportDemSwitchModeError(void)
{
    if ((uint32)STD_ON == Mcu_pDemCfgPtr->ErrorSwitchModeFailureCfg.state) {
         ...
         * "AUTOSAR_SWS_DiagnosticEventManager" document, "BSW modules calling Dem_SetEventStatus
         * can safely ignore the return value." */
        (void)Dem_SetEventStatus((Dem_EventIdType)Mcu_pDemCfgPtr->ErrorSwitchModeFailureCfg.id,
                                 DEM_EVENT_STATUS_FAILED);
    }
}
#endif
