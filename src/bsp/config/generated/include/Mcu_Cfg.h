...
#define MCU_CONFIG_EXT MCU_CONFIG_VS_0_PB
...
/**
 * @brief            Pre-processor switch for enabling the default error detection and reporting to
 * the DET. The detection of default errors is configurable (ON / OFF) at pre-compile time.
 */
#define MCU_DEV_ERROR_DETECT (STD_OFF)
/**
 * @brief            Pre-processor switch to enable/disable the API to read out the modules version
 * information.
 */
...
/**
 * @brief            If this parameter is set to FALSE, the clock initialization has to be disabled
 * from the MCU driver.
 */
...
/**
 * @brief            The user callout reset is/is not available (STD_ON/STD_OFF) - called by MCU
 * right before Mcu_PerformReset().
 */
...
/**
 * @brief            Enable the usage of Non-Autosar API Mcu_GetPeripheral_State() for getting infos
 * about peripheral state from MC_ME module.
 */
...
/**
 * @brief            Enable the usage of Non-Autosar API Mcu_GetPowerDomain_Status. Get PMU state:
 * operable or not.
 */
...
/**
 * @brief           This define controls whether the MCU driver will configure and supervise the
 * Application Extension Subsystem.
 */
 ...
 *               If this parameter has been configured to 'TRUE' the Mcu driver can be executed from
...
/**
 * @brief  The function Mcu_GetResetRawValue shall return an implementation specific value which
 * does not correspond to a valid value of the reset status register and is not equal to 0 if this
...
/**
 * @brief            The type Mcu_ResetType, represents the different reset that a specified MCU can
 * have.
 * @details          The MCU module shall provide at least the values MCU_POWER_ON_RESET and
 ...
 * @details          This is used to specify the entry and exit point of the flash and ram
 ...
 * @details          The type of Mcu_SRAMRetenConfigType is an enumeration with the following
...
/**
 * @brief            The Mcu_ModeType specifies the identification (ID) for a MCU mode, configured
...
/**
 * @brief            The Mcu_RamSectionType specifies the identification (ID) for a RAM section,
 * configured via the configuration structure. The type shall be uint8, uint16 or uint32, based on
 * best performance.
...
#if (MCU_INIT_CLOCK == STD_ON)
/**
 * @brief          Defines the identification (ID) for clock setting configured via the
...
/**
 * @brief            The type Mcu_RawResetType specifies the reset reason in raw register format,
