 ...
 * user mode access SUPERVISOR_MODE_SOFT_LOCKING:  Locks the access to the registers only for
 * supervisor mode
 *
 ...
 *        If this parameter has been configured to STD_ON, the Dio driver code can be executed from
 ...
 *        If the current platform implements REG_PROT for GPIO IP, this parameter will be defined,
 * and will enable REG_PROT configuration for GPIO IP in DIO drvier
 */
...
typedef struct {
    ...
    const uint32*
        pau32Dio_ChannelToPartitionMap; /**< @brief Pointer to channel to partition mapping */
