...
typedef enum {
    ...
    ETH_RECEIVED_MORE_DATA_AVAILABLE /**< A frame received and at least another one in the queue
                                        detected */
} Eth_RxStatusType;
...
/**
 * @brief This type defines the Ethernet link state. The link state changes after an Ethernet cable
 * gets plugged in and the transceivers on both ends negotiated the transmission parameters (i.e.
...
/**
 * @brief This type defines the Ethernet baud rate. The baud rate gets either negotiated between the
 * connected transceivers or has to be configured.
...
/**
 * @brief This type defines the Ethernet duplex mode. The duplex mode gets either negotiated between
 * the connected transceivers or has to be configured.
...
typedef enum {
    ...
    /**< @brief Wake-up on data line (WUP = wake up pulse) detected while link is down of the
corresponding Ethernet hardware (e.g. PHY). Only available if EthTrcvWakeupSleepOn DatalineEnabled
is set to TRUE */
    ETHTRCV_WUR_WODL_WUP = (uint8)0x8U,
    /**< @brief Wake-up on data line (WUR = wake up request) detected while link is active of the
used Ethernet hardware (e.g. PHY). Only available if EthTrcvWakeupSleepOnDataline Enabled is set to
TRUE */
    ETHTRCV_WUR_WODL_WUR = (uint8)0x9U,
    /**< @brief Device internal transfer of wake-up on data line from a neighboring PHY. Only
possible if Ethernet hardware is compliant to OA TC10. Only available if EthTrcvWakeupSleepOn
DatalineEnabled is set to TRUE. */
...
typedef enum {
    ...
    /**< @brief cable diagnostics has detected wrong polarity of the "Ethernet physical+" or
       "Ethernet physical-" lines */
    ETHTRCV_CABLEDIAG_WRONG_POLARITY = (uint8)0x5U

} EthTrcv_CableDiagResultType;

/**
 * @brief  Status supervision used for Development Error Detection. The state shall be available for
 * debugging.
 ...
 */
typedef enum {
    ETHSWT_MACLEARNING_HWDISABLED = (uint8)0x0U, /**< @brief If hardware learning disabled, the
                                                    switch must not learn new MAC addresses */
    ETHSWT_MACLEARNING_HWENABLED = (uint8)0x1U, /**< @brief If hardware learning enabled, the switch
                                                   learns new MAC addresses */
    ETHSWT_MACLEARNING_SWENABLED =
        (uint8)0x2U /**< @brief If software learning enabled, the hardware learning is disabled and
                       the switch forwards packets  with an unknown source address to a host CPU */
} EthSwt_MacLearningType;

/**
 * @brief Type to request or obtain the port mirroring state (enable/disable) for a particular port
 * mirror configuration per Ethernet switch.
...
typedef enum {
    ...
    ETHSWT_MGMT_OBJ_OWNED_BY_ETHSWT =
        (uint8)0x1U, /**< @brief Object used and EthSwt collects needed data */
    ETHSWT_MGMT_OBJ_OWNED_BY_UPPER_LAYER =
        (uint8)0x2U /**< @brief Object used and the upper layer does calculations */
} EthSwt_MgmtOwner;
...
typedef struct {
    Eth_TimeIntDiffType
        IngressTimeStampDelta; /**< @brief   IngressTimeStampSync2 -IngressTimeStampSync1 */
    Eth_TimeIntDiffType
        OriginTimeStampDelta; /**< @brief OriginTimeStampSync2[FUP2]-OriginTimeStampSync1[FUP1] */
} Eth_RateRatioType;
...
/**
 * @brief The EthSwt_PortMirrorCfgType specify the port mirror configuration which is set up per
 * Ethernet switch. The configuration is written to the Ethernet switch driver by calling
 * EthSwt_WritePortMirrorConfiguration. One port mirror configuration is maintained per Ethernet
 * Switch.
 */
typedef struct {
    uint8 srcMacAddrFilter[6U]; /**< @brief Specifies the source MAC address
                                   [0..255,0..255,0..255,0..255,0..255,0..255] that should be
                                   mirrored. If set to 0,0,0,0,0,0, no source MAC address filtering
                                   shall take place. */
    uint8 dstMacAddrFilter[6U]; /**< @brief Specifies the destination MAC address
                                   [0..255,0..255,0..255,0..255,0..255,0..255] that should be
                                   mirrored. If set to 0,0,0,0,0,0, no destination MAC address
                                   filtering shall take place. */
    uint16 VlanIdFilter; /**< @brief Specifies the VLAN address 0..4094 that should be mirrored. If
                            set to 65535, no VLAN filtering shall take place. */
    uint8 MirroringPacketDivider; /**< @brief Divider if only a subset of received frames should be
                                     mirrored. E.g. MirroringPacketDivider = 2 means every second
                                     frames is mirrored */
    uint8 MirroringMode; /**< @brief specifies the mode how the mirrored traffic should be tagged :
                            0x00 == No VLAN retagging; 0x01 == VLAN retagging; 0x03 == VLAN Double
                            tagging */
    uint32
        TrafficDirectionIngressBitMask; /**< @brief Specifies the bit mask of Ethernet switch
                                           ingress port traffic direction to be mirrored. The bit
                                           mask is calculated depending of the values of
                                           EthSwtPortIdx. (e.g. set EthSwtPortIdx == 2 =>
                                           TrafficDirectionIngressBitMask = 0b0000 0000 0000 0000
                                           0000 0000 0000 0100). 0b0 == enable ingress port
                                           mirroring 0b1 == disable ingress port mirroring Example:
                                           TrafficDirectionIngressBitMask = 0b0000 0000 0000 0000
                                           0000 0000 0000 0100 => Ingress traffic mirroring is
                                           enabled of Ethernet switch port with EthSwtPortIdx=2 */
    uint32
        TrafficDirectionEgressBitMask; /**< @brief Specifies the bit mask of Ethernet switch egress
                                          port traffic direction to be mirrored. The bit mask is
                                          calculated depending of the values of EthSwtPortIdx. (e.g.
                                          set EthSwtPortIdx == 2 => TrafficDirectionEgressBitMask =
                                          0b0000 0000 0000 0000 0000 0000 0000 0100). 0b0 == enable
                                          egress port mirroring 0b1 == disable egress port mirroring
                                          Example: TrafficDirectionEgressBitMask = 0b0000 0000 0000
                                          0000 0000 0000 0000 0001 => Egress traffic mirroring is
                                          enabled of Ethernet switch port with EthSwtPortIdx=0 */
    uint8 CapturePortIdx; /**< @brief Specifies the Ethernet switch port which capture the mirrored
                             traffic */
    uint16 ReTaggingVlanId; /**< @brief Specifies the VLAN address 0..4094 which shall be used for
                               re-tagging if MirroringMode is set to 0x01 (VLAN re-tagging). If the
                               value is set to 65535, the value shall be ignored, because the VLAN
                               address for re-tagging is provided by the Ethernet switch
                               configuration */
    uint16 DoubleTaggingVlanId; /**< @brief Specifies the VLAN address 0..4094 which shall be used
                                   for double-tagging if MirroringMode is set to 0x02 (VLAN double
                                   tagging). If the value is set to 65535, the value shall be
                                   ignored, because the VLAN address for double tagging is provided
                                   by the Ethernet switch configuration */
} EthSwt_PortMirrorCfgType;

/**
 * @brief  Will be set from EthSwt and marks EthSwt_MgmtObject as valid or not. So the upper layer
 * will be able to detect inconsistencies.
 */
typedef struct {
    Std_ReturnType IngressTimestampValid; /**< @brief IngressTimestampValid shall be set to E_NOT_OK
                                             if ingress timestamp is not available */
    Std_ReturnType EgressTimestampValid;  /**< @brief EgressTimestampValid shall be set to E_NOT_OK
                                             if ingress timestamp is not available. */
    Std_ReturnType MgmtInfoValid; /**< @brief MgmtInfoValid shall be set to E_NOT_OK if ingress
                                     timestamp is not available(e.g. timeout). */
} EthSwt_MgmtObjectValidType;

/**
 * @brief  Provides information about all struct member elements. The ownership gives information
 * whether EthSwt has finished its activities in providing all struct member elements.
 */
typedef struct {
    EthSwt_MgmtObjectValidType
        Validation; /**< @brief The validation information for the mgmt_obj. */
    Eth_TimeStampType
        IngressTimestamp;              /**< @brief The ingress timestamp value out of the switch. */
    Eth_TimeStampType EgressTimestamp; /**< @brief The egress timestamp value out of the switch. */
    EthSwt_MgmtInfoType
        MgmtInfo; /**< @brief Received/Transmitted Management information of the switches. */
