...
/**
 * @brief According to SWS_Mem_00070: In case a service is not relevant for a specific memory device
 * technology, the service shall always return E_MEM_SERVICE_NOT_AVAIL. The return code is
 * implementation-defined (see SWS_Std_00011 and SWS_Std_00005).
 * @implements SymbolDefinitions_enum
 */
 ...
 *        name clashes and redefinition problems, the symbols have to be defined in the following
 * way (approved within implementation).
 */
 ...
 */
typedef enum {
    STD_TRANSFORMER_UNSPECIFIED =
        (uint8)0x00, /**< @brief Transformer of a unspecified transformer class.*/
    ...
    STD_TRANSFORMER_CUSTOM =
        (uint8)0xFF /**< @brief Transformer of a custom class not standardized by AUTOSAR.*/
} Std_TransformerClass;

/**
 * @brief Std_TransformerError represents a transformer error in the context of a certain
 * transformer chain.
 *
 * @implements Std_TransformerError_structure */
typedef struct {
    Std_TransformerErrorCode
        errorCode; /**< @brief The specific meaning of the values of Std_TransformerErrorCode is to
                   be seen for the specific transformer chain for which the data type represents the
                   transformer error. */
 ...
 * message and the type of the message result of a transformer. - At the time being, this is limited
 * to the types used for C/S communication (i.e., REQUEST and RESPONSE and OK and ERROR)
 *
 * @implements Std_ExtractProtocolHeaderFieldsType_structure
 */
typedef Std_ReturnType (*Std_ExtractProtocolHeaderFieldsType)(
    const uint8* buffer, /**< @brief Buffer allocated by the RTE, where the transformed data has to
                            be stored by the transformer */
    uint32 bufferLength, /**< @brief Length of the buffer */
    Std_MessageTypeType* messageType,    /**< @brief Canonical representation of the message type
                                            (extracted from the transformers protocol header)*/
    Std_MessageResultType* messageResult /**< @brief Canonical representation of the message result
                                            type (extracted from the transformers protocol header)*/
);
