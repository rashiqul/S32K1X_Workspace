...
typedef enum {
    ...
    CAN_ERRORSTATE_PASSIVE, /**< @brief The CAN controller takes part in communication, but does not
...
/**
 * @brief
 * @details   This type shall be used to control the CAN transceiver concerning wake up events and
 * wake up notifications. According to [SWS_CanTrcv_00164] it should be present in
 * Can_GeneralTypes.h
 */
typedef enum {
    CANTRCV_WUMODE_ENABLE =
        0x00, /* The notification for wakeup events is enabled on the addressed transceiver. */
    CANTRCV_WUMODE_DISABLE =
        0x01, /* The notification for wakeup events is disabled on the addressed transceiver. */
 ...
 */
typedef enum {
    CANTRCV_WU_ERROR = 0, /* Due to an error wake up reason was not detected. This value may only be
                             reported when error was reported to DEM before. */
    CANTRCV_WU_NOT_SUPPORTED =
        1, /* The transceiver does not support any information for the wake up reason.  */
    CANTRCV_WU_BY_BUS =
        2, /* The transceiver has detected, that the network has caused the wake up of the ECU. */
    CANTRCV_WU_INTERNALLY = 3, /* The transceiver has detected, that the network has woken up by the
                                  ECU via a request to NORMAL mode. */
    CANTRCV_WU_RESET =
        4, /* The transceiver has detected, that the "wake up" is due to an ECU reset. */
    CANTRCV_WU_POWER_ON = 5, /* The transceiver has detected, that the "wake up" is due to an ECU
                                reset after power on. */
    CANTRCV_WU_BY_PIN = 6,   /* The transceiver has detected a wake-up event at one of the
                                transceiver's pins (not at the CAN bus). */
    CANTRCV_WU_BY_SYSERR = 7 /* The transceiver has detected, that the wake up of the ECU was caused
                                by a HW related device failure. */
} CanTrcv_TrcvWakeupReasonType;

/**
 * @brief        CAN Controller Error Types of operation.
 * @details      The enumeration represents a superset of CAN Error Types which typical CAN HW is
...
typedef enum {
    ...
    CAN_ERROR_BIT = 0x03, /* The HW reports a CAN bit error but cannot report distinguish between
                             CAN_ERROR_BIT_MONITORING1 and CAN_ERROR_BIT_MONITORING0 */
    ...
    CAN_ERROR_OVERLOAD = 0x07, /* CAN overload detected via an overload frame. Indicates that the
                                  receive buffers of a receiver are full */
 ...
 * @{
 * @brief          Can_IdType
 * @details        Represents the Identifier of an L-PDU. The two most significant bits specify
 * theframe type: -00 CAN message with Standard CAN ID -01 CAN FD frame with Standard CAN ID -10 CAN
...
/** @brief This type extends the classical Can_PduType with a larger PDU length, the CanXL_Params
and a sec to indicate simple or extended content. */
typedef struct {
...
/** @brief This type defines a data structure which provides a CAN XL Hardware Object Handle
including its corresponding CAN Controller and therefore CanDrv as well as the specific CAN XL
parameters. */
typedef struct {
    /** @brief Pointer to CAN XL params. */
    CanXL_Params* XLParams;
