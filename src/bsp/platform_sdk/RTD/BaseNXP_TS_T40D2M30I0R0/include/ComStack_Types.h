...
typedef enum {
    ...
    ICOM_SWITCH_E_FAILED =
        1 /**< @brief The activation of Pretended Networking was not successful */
...
typedef struct {
    ...
    PduLengthType TxTpDataCnt; /**< Offset from the current position which identifies the number of
                                  bytes to be retransmitted. */
} RetryInfoType;

/**
 * @brief   Variables of the type IcomConfigIdType defines the configuration ID. An ID of 0 is the
 *          default configuration. An ID greater than 0 shall identify a configuration for Pretended
 * Networking.
