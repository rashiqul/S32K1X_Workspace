...
#define REG_RMW32(address, mask, value)                                                            \
    (REG_WRITE32((address), ((REG_READ32(address) & ((uint32_t) ~((uint32_t)(mask)))) |            \
                             ((uint32_t)(value)))))
#endif
...
typedef enum {
    ...
    CAN0_ORed_IRQn =
        78, /**< CAN0 OR'ed Bus in Off State/ Interrupt indicating Transmit Error Counter transition
               from less than 96 to greater than equal to 96/ Interrupt indicating Receive Error
               Counter transition from less than 96 to greater than equal to 96.  */
    CAN0_Error_IRQn =
        79, /**< CAN0 Interrupt indicating that errors were detected on the CAN bus/ Interrupt
               indicating that errors were detected on the CAN bus/ Interrupt indicating that errors
               were detected on the CAN bus for FD messages in the Fast Bit Rate region/ Interrupt
               asserted when Pretended Networking operation is enabled, and no-reception of a
               matching message for a defined quantity of time during low power mode */
    CAN0_Wake_Up_IRQn =
        80, /**< CAN0 Interrupt asserted when Pretended Networking operation is enabled, and a valid
               message matches the selected filter criteria during Low Power mode/ Interrupt
               asserted when Pretended Networking operation is enabled, and no-reception of a
               matching message for a defined quantity of time during low power mode */
    ...
    CAN1_ORed_IRQn =
        85, /**< CAN1 OR'ed Bus in Off State/ Interrupt indicating Transmit Error Counter transition
               from less than 96 to greater than equal to 96/ Interrupt indicating Receive Error
               Counter transition from less than 96 to greater than equal to 96 */
    CAN1_Error_IRQn = 86, /**< CAN1 Interrupt indicating that errors were detected on the CAN bus/
                             Interrupt indicating that errors were detected on the CAN bus for FD
                             messages in the Fast Bit Rate region */
